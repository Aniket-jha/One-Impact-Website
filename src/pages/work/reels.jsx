...
import WorksStyle2 from "../../components/Works-style2/work-style-2reel";
...
const WorksDark = ({works,productions,shoots,reels,productionReels,blogs}) => {
  ...
  return (
    <DarkTheme>
      ...
      <WorksHeader title="Reels" sliderRef={fixedHeader} />
...
export const getServerSideProps = async () => {
   ...
   const creativeReelQuery = `*[_type == "reels"] | order(_createdAt desc){
    _id,
    name,
     category,
       video{
  ...
  const reels= await client.fetch(creativeReelQuery)
  const productionReelQuery = `*[_type == "productionReels"] | order(_createdAt desc){
    _id,
    name,
     category,
       video{
  ...
  const productionReels =  await client.fetch(productionReelQuery)
    const blogsQuery = '*[_type == "journal"]';
  const blogs = await client.fetch(blogsQuery);
  return {
    props: { reels,productionReels,blogs }
  }
}
